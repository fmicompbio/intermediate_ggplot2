...
```{r}
ggplot(vehicles %>% group_by(class) %>% summarize(mean_hwy = mean(hwy)), 
...
```{r}
ggplot(vehicles %>% mutate(fuel_lumped = fct_lump_n(fuel, n = 3, other_level = "Other")),
       aes(x = hwy, y = cty)) +
...
a disadvantage of this approach is all elements will be rendered into that
...
Here we will illustrate two approaches to making interactive "`ggplot`-like" 
graphics. 
...
them in an ellipse (`ggforce::geom_mark_circle`) or a polygon line
(`ggalt::geom_encircle`):
...
plot. `geom_text_repel` just add's the text labels, `geom_label_repel` adds
...
ggplot(vehicles %>% dplyr::filter(year == 2005 & hwy > 30), aes(x = hwy, y = cty)) +
    ...
    ggp + theme(panel.background = element_rect(fill = "gray88"))
```
...
`show.legend` can be used to not include a single layer in the legend. Other layers
will be unaffected:

```{r}
ggplot(vehicles, aes(hwy, cty, color = fuel)) +
    geom_point(show.legend = FALSE) +
    geom_point(inherit.aes = FALSE, data = vehicles %>% dplyr::filter(make == "Ram"),
...
```{r}
## One-sided gradient
ggplot(vehicles %>% dplyr::filter(year == 2005), aes(x = hwy, y = cty, color = cyl)) +
    geom_point() + 
    scale_color_gradient(low = "white", high = "darkblue") + 
    ggp

## Two-sided gradient
ggplot(vehicles %>% dplyr::filter(year == 2005), aes(x = hwy, y = cty, color = scale(cyl))) +
    geom_point() + 
...
```{r}
## Histogram...
ggplot(vehicles %>% dplyr::filter(year == 2005), aes(x = hwy)) +
    geom_histogram(bins = 30) + 
    ggp

## ...is equivalent to
ggplot(vehicles %>% dplyr::filter(year == 2005), aes(x = hwy, y = after_stat(count))) +
    geom_histogram(bins = 30) + 
    ggp

## To get density instead:
ggplot(vehicles %>% dplyr::filter(year == 2005), aes(x = hwy, y = after_stat(density))) +
    geom_histogram(bins = 30) + 
    ggp
...
ggplot(vehicles %>% dplyr::filter(year == 2005), aes(x = hwy, y = after_stat(count/max(count)))) +
    geom_histogram(bins = 30) + 
    ggp
```
...
```{r}
g1 <- ggplot(vehicles, aes(x = fct_lump_n(fuel, n = 3, other_level = "Other"), 
                     fill = fct_lump_n(make, n = 3))) +
    geom_bar(position = "fill") +
    scale_fill_brewer(palette = "Set2", name = "Make") + 
    ggp
g2 <- ggplot(vehicles %>% dplyr::filter(year == 2005), aes(x = hwy, y = cty, color = scale(cyl))) +
    geom_point() + 
    scale_color_gradient2(low = "red", mid = "white", high = "darkblue", 
                          midpoint = 0) + 
    ggp
